...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="oceanfresh-yolo-predict",
        description="Run Ocean Fresh YOLO prediction with configurable product postprocessing.",
    )
    parser.add_argument("--config", type=str, default=None, help="Path to a JSON config file.")
    parser.add_argument("--weights", type=str, default=None, help="Path to model weights.")
    parser.add_argument("--source", type=str, default=None, help="Prediction source, e.g. image or folder.")
    ...
    parser.add_argument("--same-class-contain-suppression", action=argparse.BooleanOptionalAction, default=None)
    parser.add_argument("--contain-ratio-threshold", type=float, default=None)
    parser.add_argument("--adjacent-frame-dedup", action=argparse.BooleanOptionalAction, default=None)
    parser.add_argument("--adjacent-frame-edge-priority-rule", action=argparse.BooleanOptionalAction, default=None)
    parser.add_argument("--adjacent-frame-x-overlap-threshold", type=float, default=None)
    parser.add_argument("--adjacent-frame-delta-y", type=float, default=None)
    parser.add_argument("--adjacent-frame-height-min", type=float, default=None)
    parser.add_argument("--adjacent-frame-height-max", type=float, default=None)
    parser.add_argument("--adjacent-frame-height-confidence-offset-px", type=float, default=None)
    parser.add_argument("--adjacent-frame-non-bottom-y2-correction", type=float, default=None)
    parser.add_argument("--adjacent-frame-bottom-touch-margin-px", type=float, default=None)
    parser.add_argument("--adjacent-frame-x-tolerance", type=float, default=None)
    parser.add_argument("--export-penalty-hits", action=argparse.BooleanOptionalAction, default=None)
    parser.add_argument("--penalty-hits-dirname", type=str, default=None)
    parser.add_argument("--render-adjacent-frame-guides", action=argparse.BooleanOptionalAction, default=None)
...
def build_run_config(config: dict, config_path: Path | None, weights_path: Path, source_path: Path, save_dir: Path) -> dict:
    payload = dict(config)
    payload["config_path"] = str(config_path) if config_path else None
    payload["weights"] = str(weights_path)
...
def resolve_previous_frame_y1_window(
    *,
    next_y2: float,
    ...
    next_y1 = float(next_det["y1"])
    next_height_px = max(0.0, float(next_det["y2"]) - next_y1)
    next_edge_type = detect_vertical_edge_type(
        next_y1,
        float(next_det["y2"]),
        int(next_frame["image_height"]),
        float(edge_touch_margin_px),
    )
    next_is_top_touching = next_edge_type in {"top", "top_bottom"}

    if next_is_top_touching:
        previous_y1_min = float(next_y2) + float(delta_y) - float(height_max)
    else:
        previous_y1_min = float(next_y2) + float(delta_y) - next_height_px - float(non_bottom_y2_correction)

    previous_y1_max = float(next_y2) + float(delta_y) - float(height_min)
    return previous_y1_min, previous_y1_max
...
def apply_adjacent_frame_dedup(
    ...
    *,
    ...
    for frame_index in range(len(frame_entries) - 1):
        previous_frame = frame_entries[frame_index]
        next_frame = frame_entries[frame_index + 1]
        previous_detections = list(previous_frame["detections"])
        ...
        for next_index, next_det in enumerate(next_detections):
            ...
            next_frame_height = float(next_frame["image_height"])
            next_y2_scan_max = next_frame_height - float(delta_y) + float(height_max)
            if next_y2 > next_y2_scan_max:
                continue

            for previous_index, previous_det in enumerate(previous_detections):
                if bool(previous_det.get("temporal_matched")):
                    continue
                previous_y1 = float(previous_det["y1"])
                previous_y1_min, previous_y1_max = resolve_previous_frame_y1_window(
                    next_y2=next_y2,
                    next_det=next_det,
                    ...
                    non_bottom_y2_correction=non_bottom_y2_correction,
                    edge_touch_margin_px=bottom_touch_margin_px,
                )
                if previous_y1 < previous_y1_min or previous_y1 > previous_y1_max:
                    continue
                overlap_ratio = horizontal_overlap_ratio(previous_det, next_det)
                if overlap_ratio < float(x_overlap_threshold):
                    continue
                left_dx = abs(float(next_det["x1"]) - float(previous_det["x1"]))
                right_dx = abs(float(next_det["x2"]) - float(previous_det["x2"]))
                if min(left_dx, right_dx) > float(x_tolerance):
                    continue
                y_gap_to_window = min(
                    abs(previous_y1 - previous_y1_min),
                    abs(previous_y1 - previous_y1_max),
                )
                score = (
                ...
                candidates.append((score, previous_index, next_index, overlap_ratio))

        candidates.sort(key=lambda item: item[0])
        used_previous: set[int] = set()
        used_next: set[int] = set()
        for _score, previous_index, next_index, overlap_ratio in candidates:
            if previous_index in used_previous or next_index in used_next:
                continue
            current_det = previous_detections[previous_index]
...
def render_adjacent_frame_guide_lines(
    ...
    *,
    ...
    color = (0, 255, 255)
    y2_scan_max = float(image_height) - float(delta_y) + float(height_max)
    y = int(round(y2_scan_max))
    if 0 <= y < image_height:
        cv2.line(image, (0, y), (image_width - 1, y), color, 2, cv2.LINE_AA)
    text = f"y2-first scan max: {y2_scan_max:.2f}"
            ...
            detections.append(
                {
                    ...
                    "y2": y2,
                    "edge_type": None,
                    "actual_vertical_edge_type": None,
                    ...
                    "actual_vertical_edge_span_score": None,
                    "horizontal_width_height_ratio": None,
                    "horizontal_top_gap": None,
                    "horizontal_bottom_gap": None,
                    "horizontal_min_edge_gap": None,
                    "horizontal_edge_span_score": None,
                    "horizontal_rule_edge_type": None,
                    "horizontal_edge_touch_px": None,
                    "horizontal_flat_ratio_threshold_short": None,
                    "horizontal_flat_ratio_threshold": None,
                    "horizontal_edge_span_threshold_short": None,
                    "horizontal_edge_span_threshold": None,
                    "horizontal_penalty_factor": None,
                    "intact_aspect_ratio_threshold": None,
                    "edge_span_threshold": None,
                    "edge_span_threshold_thin": None,
                    "defective_penalty_factor": None,
...
def finalize_frame_entry(
    *,
    ...
    render_adjacent_frame_guides: bool,
        ...
        summary_items.append(
            {
                ...
                "x_center_px": det["x_center_px"],
                "y_center_px": det["y_center_px"],
            }
        )
    ...
    write_parameters = [cv2.IMWRITE_PNG_COMPRESSION, 1] if output_path.suffix.lower() == ".png" else []
    if not cv2.imwrite(str(output_path), image, write_parameters):
...
def save_adjusted_predictions(
    *,
    ...
    for result in results:
        ...
        frame_entries.append(build_frame_entry(
            result=result,
            output_path=output_path,
            same_class_contain_suppression=same_class_contain_suppression,
            contain_ratio_threshold=contain_ratio_threshold,
        ))
    ...
    for frame_entry in frame_entries:
        summary.extend(finalize_frame_entry(
            frame_entry=frame_entry,
            labels_dir=labels_dir,
            save_txt=save_txt,
            save_conf=save_conf,
            penalty_hits_dir=penalty_hits_dir,
            render_adjacent_frame_guides=render_adjacent_frame_guides,
            adjacent_frame_dedup=adjacent_frame_dedup,
            adjacent_frame_delta_y=adjacent_frame_delta_y,
            adjacent_frame_height_max=adjacent_frame_height_max,
        ))
...
def main() -> None:
    ...
    config_path = Path(args.config).resolve() if args.config else script_dir / "predict_youge.json"
    ...
    weights_value = predict_config.get("weights")
    if not weights_value:
        raise ValueError("weights 不能为空。")
    source_value = predict_config.get("source")
    if not source_value:
        raise ValueError("source 不能为空。")
    imgsz_value = predict_config.get("imgsz")
    if imgsz_value is None:
        raise ValueError("imgsz 不能为空，必须由模型版本的训练分辨率提供。")
    try:
        image_size = int(imgsz_value)
    except (TypeError, ValueError) as exc:
        raise ValueError("imgsz 必须是正整数。") from exc
    if image_size <= 0:
        raise ValueError("imgsz 必须是正整数。")
    predict_config["imgsz"] = image_size
    ...
    project_dir = Path(str(predict_config.get("project") or "runs/predict")).expanduser()
    if not project_dir.is_absolute():
        project_dir = (config_path.parent if config_path else script_dir) / project_dir
    project_dir = project_dir.resolve()
    run_name = str(predict_config.get("name") or "yolo_predict")
    ...
    should_use_phase2 = any(
        bool(predict_config.get(key))
        for key in ("same_class_contain_suppression", "adjacent_frame_dedup")
    )

    with (save_dir / "run_config.json").open("w", encoding="utf-8") as handle:
        json.dump(
            build_run_config(predict_config, config_path if config_path.exists() else None, weights_path, source_path, save_dir),
            handle,
            ensure_ascii=False,
            indent=2,
        )
    ...
    print(f"Prediction finished. Results saved to: {save_dir}")
